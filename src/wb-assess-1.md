...
```
console.log() is a built in js function that logs/prints to the console. return is the keyword used to end function execution and do the work of the function as desired by the programmer
```

**_What's the difference between defining and calling a function?_**

```
Defining a function is where the programmer tells the computer some kind of work he/she wants it to do at somepoint. Calling a function is where you actually tell the computer to do that work defined in the function.
...
```
charactors surrounded by "",'' or ``. Used to write text in JS. AKA string of text
string example:
"Hello, world"
```
...
```
Generally numbers as everyone knows them. Can be positive or negative 
```

**_Boolean_**

```
Boolean's can be either, true or false. Used based on conditional logic.
```

**_Array_**

```
Arrays are most commonly used to create lists. Although they can be empty and used for many things. They are surrounded by []. They are ordered and you can access individual things inside of an array
```
